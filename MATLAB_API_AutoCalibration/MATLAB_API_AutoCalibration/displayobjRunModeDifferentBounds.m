function displayobjRunModeDifferentBounds(inbytes,handles)
...
if (size(different,1) > 40000) % Same process as the calibration function, ensuring the image is different from the previous to reduce repeats
...
%    previmg = img;
    ...
    switch input
        case 10 %Blue off, red on
            if ((size(find(hbytes ~= B),2) < 40000) || (size(find(hbytes ~= C),2) < 40000)) % Same idea as in the calibration mode, checking both frames surrounding the current frame
                pause(0.001);
            else
            A = hbytes;
            Aimg = img;
        ...
        case 11 %red off, green on
            if ((size(find(hbytes ~= A),2) < 40000) || (size(find(hbytes ~= C),2) < 40000))
                pause(0.001);
            else
            B = hbytes;
            Bimg = img;
        ...
        case 12 %green off, blue on
            if ((size(find(hbytes ~= A),2) < 40000) || (size(find(hbytes ~= B),2) < 40000))
                pause(0.001);
            else
            C = hbytes;
            Cimg = img;
